...
RISCV_PASSWORD=$(/usr/bin/openssl passwd -crypt 'riscv')
TIMEZONE='UTC'
...
cat <<-EOF > "${TARGET_DIR}${CONFIG_SCRIPT}"
  ...
  # riscv-specific configuration
  echo ">>>> ${CONFIG_SCRIPT_SHORT}: Creating riscv user.."
  /usr/bin/useradd --password ${RISCV_PASSWORD} --create-home --user-group riscv
  echo ">>>> ${CONFIG_SCRIPT_SHORT}: Configuring sudo.."
  echo 'Defaults env_keep += "SSH_AUTH_SOCK"' > /etc/sudoers.d/11_riscv
  echo 'riscv ALL=(ALL) NOPASSWD: ALL' >> /etc/sudoers.d/11_riscv
  /usr/bin/chmod 0440 /etc/sudoers.d/11_riscv
